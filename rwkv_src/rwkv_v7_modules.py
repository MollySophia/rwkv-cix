...
from rwkv_src.modules import *

def l2norm_workaround(x):
    return x * torch.rsqrt(x.pow(2).sum(dim=-1, keepdim=True) + 1e-7)
...
class Rwkv7SelfAttention(nn.Module):
    def __init__(self, state_dict, hidden_size, head_size, layer_id=0, rescale_layer=0, custom_wkv=False):
        ...
        self.lerp_r = Lerp(nn.Parameter(state_dict[prefix + 'x_r']))
        self.lerp_w = Lerp(nn.Parameter(state_dict[prefix + 'x_w']))
        self.lerp_k = Lerp(nn.Parameter(state_dict[prefix + 'x_k']))
        self.lerp_v = Lerp(nn.Parameter(state_dict[prefix + 'x_v']))
        self.lerp_a = Lerp(nn.Parameter(state_dict[prefix + 'x_a']))
        self.lerp_g = Lerp(nn.Parameter(state_dict[prefix + 'x_g']))
        ...
        self.ln_x = nn.LayerNorm(self.head_size, eps=64e-5, elementwise_affine=False)
    ...
    def forward(self, x, state1, state2, v_first):
        ...
        # kk = self.l2norm(kk)
        kk = l2norm_workaround(kk)
        # kk = kk * torch.rsqrt(kk.pow(2).mean(dim=-1, keepdim=True) + 1e-6)
        ...
        # x = F.instance_norm(x.view(1, self.num_heads, 1, -1), eps=64e-5).view(1, -1)
